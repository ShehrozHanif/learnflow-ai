...
function LoginPage({ onLogin, onBack }) {
  ...
  const attempt = (role) => {
    ...
    setTimeout(() => { setLoading(null); onLogin(role); }, 800);
  };
  const submit = () => {
    ...
    <div style={{ minHeight:"100vh", background:"#0F172A", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"24px 16px", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ width:"100%", maxWidth:380, position:"relative", zIndex:1, animation:"fadein 0.3s ease" }}>
        ...
        <div style={{ background:"rgba(30,41,59,0.65)", border:"1px solid rgba(148,163,184,0.1)", borderRadius:18, padding:"24px 22px", backdropFilter:"blur(16px)", boxShadow:"0 24px 48px rgba(0,0,0,0.4)" }}>
          <h1 style={{ fontSize:17, fontWeight:700, color:"#F1F5F9", marginBottom:3, textAlign:"center" }}>Welcome back</h1>
          <p style={{ fontSize:12, color:"#64748B", textAlign:"center", marginBottom:22 }}>Sign in to continue learning</p>
          ...
          <button onClick={submit} disabled={!!loading} style={{ width:"100%", padding:"11px", borderRadius:10, border:"none", background:loading?"rgba(59,130,246,0.25)":"linear-gradient(135deg,#2563EB,#3B82F6)", color:"white", fontSize:14, fontWeight:600, cursor:loading?"default":"pointer", boxShadow:loading?"none":"0 4px 16px rgba(59,130,246,0.3)", display:"flex", alignItems:"center", justifyContent:"center", gap:8, marginBottom:16 }}>
            {loading==="student" ? <><span style={{width:13,height:13,border:"2px solid rgba(255,255,255,0.35)",borderTopColor:"white",borderRadius:"50%",display:"inline-block",animation:"spin 0.7s linear infinite"}}/>Signing in…</> : "Sign In"}
          </button>

          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
            <div style={{ flex:1, height:1, background:"rgba(148,163,184,0.08)" }}/>
            <span style={{ fontSize:11, color:"#334155", whiteSpace:"nowrap" }}>or jump in as</span>
            <div style={{ flex:1, height:1, background:"rgba(148,163,184,0.08)" }}/>
          </div>

          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            <button onClick={()=>attempt("student")} disabled={!!loading}
            ...
            <button onClick={()=>attempt("teacher")} disabled={!!loading}
...
function StudentDashboard() {
  ...
  const send = () => {
    ...
    setTimeout(()=>{ setTyping(false); setMsgs(p=>[...p,{id:Date.now()+1,role:"ai",agent:"Concepts Agent",text:"Great question! Try it in the Code tab and come back if you need help debugging."}]); },1700);
  };
  const TABS=[{id:"chat",label:"AI Chat",icon:"💬"},{id:"code",label:"Code",icon:"💻"},{id:"progress",label:"Progress",icon:"📊"}];
  const renderMsg=(msg)=>{
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",background:"rgba(15,23,42,0.7)",borderBottom:"1px solid rgba(148,163,184,0.07)",flexShrink:0,padding:"0 4px"}}>
        {TABS.map(t=>{const active=tab===t.id;return <button key={t.id} onClick={()=>setTab(t.id)} style={{display:"flex",alignItems:"center",gap:6,padding:"0 12px",height:42,border:"none",borderBottom:active?"2px solid #3B82F6":"2px solid transparent",cursor:"pointer",fontSize:13,fontWeight:active?600:400,color:active?"#93C5FD":"#64748B",background:"transparent",transition:"all 0.15s",whiteSpace:"nowrap"}}><span>{t.icon}</span>{t.label}</button>;})}
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:5,padding:"0 12px"}}>
          <div style={{width:48,height:3,borderRadius:99,background:"rgba(148,163,184,0.1)",overflow:"hidden"}}><div style={{height:"100%",width:"64%",borderRadius:99,background:"linear-gradient(90deg,#3B82F6,#818CF8)"}}/></div>
          <span style={{fontSize:11,fontWeight:700,color:"#60A5FA"}}>64%</span>
        </div>
      </div>
      {tab==="chat"&&(
      ...
      {tab==="code"&&(
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          <div style={{flex:1,overflowY:"auto",background:"#181E2D",padding:"10px 0",fontFamily:"monospace"}}>
            {STARTER.split("\n").map((line,i)=>(
              <div key={i} style={{display:"flex",lineHeight:"22px"}} onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.02)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                <span style={{userSelect:"none",width:36,textAlign:"right",paddingRight:12,fontSize:12,color:"#3E4451",flexShrink:0}}>{i+1}</span>
                <span style={{fontSize:12,flex:1,color:"#ABB2BF"}} dangerouslySetInnerHTML={{__html:hl(line)||"&nbsp;"}}/>
              </div>
            ))}
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"9px 13px",borderTop:"1px solid rgba(148,163,184,0.07)",background:"rgba(15,23,42,0.7)",flexShrink:0}}>
            <button onClick={()=>{setRunState("running");setOutput("");setTimeout(()=>{setRunState("done");setOutput("Hello, LearnFlow!\n7\n9");},1000);}} style={{display:"flex",alignItems:"center",gap:5,padding:"7px 13px",borderRadius:8,border:"none",background:runState==="running"?"rgba(16,185,129,0.12)":"linear-gradient(135deg,#059669,#10B981)",color:runState==="running"?"#34D399":"white",fontSize:13,fontWeight:600,cursor:"pointer",transition:"all 0.18s"}}>
          ...
          <div style={{maxWidth:680,margin:"0 auto"}}>
            <div style={{background:"rgba(30,41,59,0.4)",border:"1px solid rgba(148,163,184,0.07)",borderRadius:16,padding:"20px",marginBottom:14,display:"flex",flexWrap:"wrap",gap:18,alignItems:"center"}}>
              <div style={{position:"relative",width:96,height:96,flexShrink:0}}>
                <Ring pct={64} size={96} stroke={7}/>
                <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
                  <span style={{fontSize:20,fontWeight:800,color:"#F1F5F9"}}>64%</span>
                  <span style={{fontSize:9,color:"#64748B",textTransform:"uppercase",letterSpacing:"0.08em"}}>Mastery</span>
                </div>
              </div>
              <div style={{flex:1,minWidth:160}}>
                <div style={{fontSize:15,fontWeight:700,color:"#F1F5F9",marginBottom:2}}>Maya Chen</div>
                <div style={{fontSize:12,color:"#64748B",marginBottom:12}}>Module 3 · Data Structures</div>
                <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                  {[{e:"🔥",v:"5d",l:"Streak"},{e:"📝",v:"23",l:"Exercises"},{e:"🎯",v:"78%",l:"Quiz Avg"}].map(s=>(
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:9}}>
              {TOPICS.map(t=>{const ls=LEVEL[t.level];return(
    ...
    <div style={{flex:1,overflowY:"auto",padding:"18px 14px"}}>
      <div style={{maxWidth:940,margin:"0 auto",display:"flex",flexDirection:"column",gap:18}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:10}}>
          {[{label:"Total Students",value:"24",accent:"#3B82F6",e:"👥"},{label:"Avg Mastery",value:"67%",accent:"#10B981",e:"📊"},{label:"Active Now",value:"12",accent:"#10B981",e:"⚡",pulse:true},{label:"Struggling",value:"3",accent:"#F43F5E",e:"⚠️"}].map(c=>(
        ...
        <div style={{background:"rgba(30,41,59,0.4)",border:"1px solid rgba(148,163,184,0.07)",borderRadius:14,overflow:"hidden"}}>
          ...
          :visible.map((a,i)=>(
            <div key={a.id} style={{borderBottom:i<visible.length-1?"1px solid rgba(148,163,184,0.05)":"none",borderLeft:"3px solid #F43F5E",padding:"13px 15px",display:"flex",flexDirection:"column",gap:9}}>
              <div style={{display:"flex",alignItems:"center",gap:9}}>
                ...
                <button onClick={()=>setDismissed(p=>[...p,a.id])} style={{padding:"4px 7px",borderRadius:6,border:"1px solid rgba(148,163,184,0.1)",background:"transparent",color:"#475569",fontSize:11,cursor:"pointer",flexShrink:0}}>✕</button>
              </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
                {[{l:"View Code",c:"#3B82F6"},{l:"Send Help",c:"#10B981"},{l:"Assign",c:"#8B5CF6"}].map(btn=>(
                  <button key={btn.l} style={{padding:"7px 4px",borderRadius:7,border:`1px solid ${btn.c}30`,background:`${btn.c}12`,color:btn.c,fontSize:11,fontWeight:600,cursor:"pointer",textAlign:"center"}}>{btn.l}</button>
                ))}
              </div>
            </div>
          ))}
        ...
        <div style={{background:"rgba(30,41,59,0.4)",border:"1px solid rgba(148,163,184,0.07)",borderRadius:14,overflow:"hidden",marginBottom:4}}>
          <div style={{padding:"13px 15px",borderBottom:"1px solid rgba(148,163,184,0.06)"}}><div style={{fontSize:13,fontWeight:600,color:"#F1F5F9"}}>✦ Generate Exercises with AI</div><div style={{fontSize:11,color:"#475569",marginTop:1}}>Describe what you need</div></div>
          <div style={{padding:"15px"}}>
            <div style={{display:"flex",gap:8,marginBottom:generated?14:0,flexWrap:"wrap"}}>
              <input value={prompt} onChange={e=>setPrompt(e.target.value)} onKeyDown={e=>e.key==="Enter"&&(!generating)&&prompt.trim()&&(setGenerating(true),setTimeout(()=>{setGenerating(false);setGenerated(true);},1500))} placeholder="e.g. list comprehensions for beginners…" style={{flex:1,minWidth:140,background:"rgba(15,23,42,0.6)",border:"1px solid rgba(148,163,184,0.1)",borderRadius:9,padding:"9px 12px",color:"#E2E8F0",fontSize:13,fontFamily:"inherit"}}/>
              <button onClick={()=>{if(!prompt.trim()||generating)return;setGenerating(true);setTimeout(()=>{setGenerating(false);setGenerated(true);},1500);}} style={{padding:"9px 16px",borderRadius:9,border:"none",background:generating?"rgba(16,185,129,0.15)":"linear-gradient(135deg,#059669,#10B981)",color:generating?"#34D399":"white",fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:7,flexShrink:0}}>
            ...
            {generated&&<div style={{background:"rgba(15,23,42,0.6)",border:"1px solid rgba(16,185,129,0.15)",borderRadius:11,overflow:"hidden",animation:"fadein 0.25s ease"}}>
              ...
              <div style={{padding:"11px 14px",borderTop:"1px solid rgba(148,163,184,0.06)",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:9}}>
                <pre style={{fontFamily:"monospace",fontSize:12,color:"#34D399",lineHeight:1.6}}>{GENERATED_EX.expected}</pre>
                <button style={{padding:"7px 14px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#2563EB,#3B82F6)",color:"white",fontSize:12,fontWeight:600,cursor:"pointer",flexShrink:0}}>Assign to Class →</button>
              </div>
            </div>}
          </div>
        </div>
    ...
    <div style={{display:"flex",height:"100vh",background:"#0F172A",overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
        <div style={{height:50,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 14px",background:"rgba(15,23,42,0.97)",borderBottom:"1px solid rgba(148,163,184,0.08)",flexShrink:0,gap:8}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",gap:6,padding:"3px 8px 3px 3px",background:"rgba(30,41,59,0.7)",border:"1px solid rgba(148,163,184,0.08)",borderRadius:99}}>
              <div style={{width:24,height:24,borderRadius:"50%",background:grad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:"white"}}>{role==="teacher"?"R":"M"}</div>
              <span style={{fontSize:12,fontWeight:500,color:"#CBD5E1",maxWidth:80,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{role==="teacher"?"Mr. Rodriguez":"Maya Chen"}</span>
            </div>
        ...
        <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
          {activePage==="dashboard"&&role==="student"&&<StudentDashboard/>}
          {activePage==="dashboard"&&role==="teacher"&&<TeacherDashboard/>}
  ...
  return (
    <>
      ...
      {screen==="app" && role && (
        <AppShell
          role={role}
          onLogout={()=>{ setRole(null); setScreen("landing"); }}
        />
      )}
    </>
  );
